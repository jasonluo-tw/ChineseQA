...
class GatedAttnGRUCell(GRUCell):
    def __init__(self, units, **kwargs):
        super(GatedAttnGRUCell, self).__init__(units, **kwargs)

    def build(self, input_shape):
        super(GatedAttnGRUCell, self).build(input_shape)
        
        #self.state_size = [self.units] ## hidden state size, not constants

    def call(self, inputs, states, constants=None):
        ## inputs is u_Pt, uQ is all question steps
        h_tm1 = states[0]  # previous memory vP_tm1
        u_Pt = inputs # u_Pt is step-size input
        WPu = constants[0] # paragrah weights
        uQ = constants[1]  # uQ is all the question
        WQu = constants[2]  # uQ weights
        WPv = constants[3] # vP weights
        vT = constants[4]  # weights
        Wg = constants[5]
        vv = constants[6]  # download the Rnn_inputs weight (Not in article)

        WQu_uQ = K.dot(uQ, WQu)
        #print(WQu_uQ, 'A')
        WPu_uP = K.dot(K.expand_dims(u_Pt, axis=1), WPu)
        #print(WPu_uP, 'B')
        WPv_htm1 = K.dot(K.expand_dims(h_tm1, axis=1), WPv)
        #print(WPv_htm1, 'C')

        s_t = K.dot(K.tanh(WQu_uQ + WPu_uP + WPv_htm1), vT)
        s_t = K.squeeze(s_t, 2)
        a_t = softmax(s_t, axis=1)
        c_t = K.batch_dot(a_t, uQ, axes=[1, 1])
        Rnn_inputs = K.concatenate([u_Pt, c_t])
        Rnn_inputs = K.dot(Rnn_inputs, vv)
        gt = K.sigmoid(K.dot(Rnn_inputs, Wg))
        Rnn_inputs = gt * Rnn_inputs


        s_t, h1 = super(GatedAttnGRUCell, self).call(Rnn_inputs, [h_tm1])
        
        return s_t, h1

class SelfMatchGRUCell(GRUCell):
    def __init__(self, units, **kwargs):
        super(SelfMatchGRUCell, self).__init__(units, **kwargs)

    def build(self, input_shape):
        super(SelfMatchGRUCell, self).build(input_shape)

    def call(self, inputs, states, constants=None):
        ## paragraph self match
        v_Pt = inputs
        h_tm1 = states[0] # previous memory
        vP = constants[0]
        Wg = constants[1]
        ...
        WPv_vP = K.dot(vP, WPv)
        WPPv_v_Pt = K.dot(K.expand_dims(v_Pt, axis=1), WPPv)

        s_t = K.dot(K.tanh(WPv_vP + WPPv_v_Pt), vT)
        s_t = K.squeeze(s_t, 2)
        a_t = softmax(s_t, axis=1)
        c_t = K.batch_dot(a_t, vP, axes=[1, 1])

        Rnn_inputs = K.concatenate([v_Pt, c_t])
        Rnn_inputs = K.dot(Rnn_inputs, vv)
        gt = K.sigmoid(K.dot(Rnn_inputs, Wg))
        Rnn_inputs = gt * Rnn_inputs

        s_t, ht = super(SelfMatchGRUCell, self).call(Rnn_inputs, [h_tm1])

        return s_t, ht

class PointerCell(GRUCell):
    def __init__(self, units, **kwargs):
        super(PointerCell, self).__init__(units, **kwargs)

    def build(self, input_shape):
        super(PointerCell, self).build(input_shape)

    def call(self, inputs, states, constants=None):
        ## ignore the input, ther_inputs is hP
        h_tm1 = states[0]
        hP = constants[0]
        WPh = constants[1]
        Wah = constants[2]
        ...
        WPh_hP = K.dot(hP, WPh)
        Wah_h_tm1 = K.dot(K.expand_dims(h_tm1, axis=1), Wah)
        s_t = K.dot(K.tanh(WPh_hP + Wah_h_tm1), vT)
        s_t = K.squeeze(s_t, 2)
        a_t = softmax(s_t, axis=1)
        c_t = K.batch_dot(hP, a_t, axes=[1, 1])
        Rnn_inputs = c_t

        s_t, ht = super(PointerCell, self).call(Rnn_inputs, [h_tm1])
